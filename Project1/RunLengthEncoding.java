...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    	...
    	this.height = height;
    	run = new RunLength();
    	run.insertFront(new Run(0, width * height));
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    	...
    	for (int i = 0; i < runLengths.length; i++){
    		run.insertEnd(new Run(red[i], green[i], blue[i], runLengths[i]));	
    	}
  }
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
	RunIterator iter = new RunIterator(run);
    return iter;
  ...
  public PixImage toPixImage() {
    ...
    for (int i = 0; i < run.size; i++){
    	...
    	while (lengthrun > 0){
    		...
    		pix.setPixel(x, y, (short)info[1], (short)info[2], (short)info[3]);
  ...
  public String toString() {
	...
	int[] numbers = iter.next();
	while(iter.hasNext()){
		info += " length:  " + numbers[0] + " colors:  " + numbers[1];
		numbers = iter.next();
	}
	info += " length:  " + numbers[0] + " colors:  " + numbers[1];
    return info;
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    // at the end.
	  Pixel[][] pix = image.getImage();

	  Pixel current = pix[0][0];
	  int count = 0;
	  run = new RunLength();
	  ...
	  for (int y = 0; y < image.getHeight(); y++){
	    	for (int x = 0; x < image.getWidth(); x++){
	    		if (pix[x][y].getRed() != current.getRed() || pix[x][y].getGreen() != current.getGreen() || pix[x][y].getBlue() != current.getBlue()){
	    		//if (pix[x][y] != current){
	    			run.insertEnd(new Run(current.getRed(), current.getGreen(), current.getBlue(), count));
	    			count = 1;
	    			current = pix[x][y];
  ...
  public void check() {
    ...
    RunIterator iter = this.iterator();
    RunIterator iter2 = this.iterator();
    ...
    for(int i = 1; i < run.size; i++){
    	info = iter.next();
    	if (cRed == info[1] && cGreen == info[2] && cBlue == info[3]){
    		System.out.println("Consecutive runs have same RGB values.");
    	}
    	cRed = info[1];
    	cGreen = info[2];
    	cBlue = info[3];
    ...
    if (total != height * width){
    	System.out.println("Sum of run lengths does not equal number of pixels.");
    }
  }
